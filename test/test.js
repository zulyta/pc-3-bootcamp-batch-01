    ...
    describe("Depositando Ether para Random NFT", () => {
      it("Método emite evento (30 veces) ", async () => {
        ...
        for (let index = 1; index <= 30; index++) {
          ...
          /*const event = events.find(x => x.event === "DeliverNft");
          // get second arg which is the address
          const idNFT = event.args[1];
          const name = event.args[0];
          expect(name).to.equal("");*/
          //console.log(`${owner.address} name ${name}  id ${idNFT}`);
        }
      });
      ...
      it("Da vuelto cuando y gnosis recibe Ether", async () => {
          ...
          ).to.changeEtherBalances(   //SE ESPERA QUE GNOSIS TENGA 0.01
